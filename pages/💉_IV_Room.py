...
load_wcc_compounding_stats = App.load_wcc_compounding_stats
...
with st.spinner("Loading IV room workload..."):
    ...
    df_wcc = load_wcc_compounding_stats(start_date, end_date)

if df_iv.empty and df_wcc.empty:
    st.info("No IV room workload found for this date range. Upload an `IV Room Workload`, `IV Room Batching`, or `WCC Compounding Stats` file from the sidebar to get started.")
    st.stop()
...
if not df_wcc.empty:
    ...
    st.subheader("WCC Compounding Stats")
    ...
    selected_wcc_statuses = wf1.multiselect("WCC Barcode Status", status_options, default=status_options)
    component_search = wf2.text_input("WCC component/order search")
    ...
    with wc1:
        ...
        daily_wcc["compliance_rate"] = daily_wcc["compliant"] / daily_wcc["administrations"]
        st.markdown("##### Daily WCC Volume")
    ...
    if not top_components.empty:
        top_components["compliance_rate"] = top_components["compliant"] / top_components["administrations"]
        top_components["compliance_pct"] = top_components["compliance_rate"] * 100
    else:
        top_components["compliance_pct"] = pd.Series(dtype=float)
    st.markdown("##### Highest-Volume WCC Components")
    component_cols = [
        "component_name", "order_name", "administrations", "compliant",
        "non_compliant", "compliance_pct", "first_admin", "last_admin",
    ]
    st.dataframe(
    ...
    with st.expander("WCC Raw Administration Rows"):
...
if df_iv.empty:
    st.info("No standard IV room workload rows found for this date range. The WCC compounding stats above are still available.")
    st.stop()
