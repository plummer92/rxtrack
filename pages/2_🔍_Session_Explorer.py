...
# Data Unification
# ----------------------------
def is_pharmacy_work_area(value):
    text_value = str(value or "").strip().lower()
    return bool(re.search(r"\bcar\s*\d+\b|carousel|cubic|\bshelf\b|\bems\b", text_value))


px_df = df_events[['user_name', 'dt', 'device', 'event_type', 'med_desc', 'qty', 'pk']].copy() if not df_events.empty else pd.DataFrame()
if not px_df.empty:
    px_df['source'] = 'Pyxis'
    px_df.loc[px_df['device'].apply(is_pharmacy_work_area), 'source'] = 'Pharmacy'
...
def pharmacy_work_label(event_values, destinations, start_dt):
    ...
    carousel_destination = is_pharmacy_work_area(destination_text)
